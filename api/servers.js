    ...
    if (newsettings.api.client.allow.server.create == true) {
      ...
      if (req.query.name && req.query.ram && req.query.disk && req.query.cpu && req.query.egg && req.query.location) {
        ...
        if (!isNaN(ram) && !isNaN(disk) && !isNaN(cpu)) {
          if (ram2 + ram > package.ram + extra.ram + j4r.ram) return res.redirect(`${redirectlink}?err=EXCEEDRAM&num=${package.ram + extra.ram + j4r.ram - ram2}`);
          if (disk2 + disk > package.disk + extra.disk + j4r.disk) return res.redirect(`${redirectlink}?err=EXCEEDDISK&num=${package.disk + extra.disk + j4r.disk - disk2}`);
          if (cpu2 + cpu > package.cpu + extra.cpu + j4r.cpu) return res.redirect(`${redirectlink}?err=EXCEEDCPU&num=${package.cpu + extra.cpu + j4r.cpu - cpu2}`);
